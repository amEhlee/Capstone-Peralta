...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.csrf().disable().authorizeRequests()
                ...
                .and().authorizeRequests().antMatchers("/user/login", "/user/signup", "/user/auth/refreshtoken", "/item/get/**", "order/**" ,"/order/get/user/{userId}", "/order/add", "/category/{categoryId}", "category/all", "/email/send" , "/item/search/{query}").permitAll()
                .and().authorizeRequests().antMatchers("/user/**").hasAnyAuthority("ROLE_USER", "ROLE_ADMIN", "ROLE_OWNER")
                .and().authorizeRequests().antMatchers("/admin/**", "/item/add/{categoryId}", "item/addMultiple", "/item/update", "image/upload/{itemId}", "category/add/{categoryId}", "category/addMultiple", "/admin/get/allItems").hasAnyAuthority("ROLE_ADMIN", "ROLE_OWNER")
